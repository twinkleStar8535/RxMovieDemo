...
//  RequestTest
//
//  Created by iOS-林祐辰 金融研發一部 on 2025/4/18.
//
...
class RequestTest: XCTestCase {
    ...
    func test_deleteRequestSuccess_case() {
        // Given
        let expectation = XCTestExpectation(description: "Delete should succeed and parse data.")
        guard let pathURL = loadMockJSONFile(named: "requestMock") else {
            XCTFail("mock.json not found in test bundle")
            return
        }
        ...
        // When
        sut?
            .request(url: pathURL, method: .delete, data: nil, header: nil, type: MockSuccessDataClass.self)
            .subscribe(
                onNext: { _ in
                    expectation.fulfill()
                },
    ...
    func test_deleteRequestFail_invalidPath_case() {
        // Given
        let expectation = XCTestExpectation(description: "Delete should fail with invalid path.")
        let invalidURL = URL(string: "invalid://path")
        
        // When
        sut?
            .request(url: invalidURL, method: .delete, data: nil, header: nil, type: MockFailDataClass.self)
            .subscribe(
                onNext: { _ in
                    XCTFail("Should not receive any data")
                },
